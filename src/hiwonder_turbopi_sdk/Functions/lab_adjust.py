...
def getAreaMaxContour(contours):
    ...
    for c in contours:  # 历遍所有轮廓 Traverse all contours
        ...
        if contour_area_temp > contour_area_max:
            ...
            if contour_area_temp > 10:  # 只有在面积大于10时，最大面积的轮廓才是有效的，以过滤干扰 Only if the area is greater than 300, the maximum contour takes effect to file the inteference
                area_max_contour = c
...
if __name__ == '__main__':      
    ...
    camera.camera_open(correction=True) # 开启畸变矫正,默认不开启 start distortion correstion which is close by default
    while True:
        img = camera.frame
        if img is not None:
            frame = img.copy()
